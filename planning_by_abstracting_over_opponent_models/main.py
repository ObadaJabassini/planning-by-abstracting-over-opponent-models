...
from planning_by_abstracting_over_opponent_models.modeling.agent.agent_loss import AgentLoss
from planning_by_abstracting_over_opponent_models.modeling.agent.agent_model import AgentModel
...
if __name__ == '__main__':
    ...
    # pommerman
    features_extractor = FeaturesExtractor(image_size=256,
                                           conv_nb_layers=4,
                                           nb_filters=32,
                                           filter_size=3,
                                           filter_stride=1,
    ...
    state = env.reset()
    state = torch.from_numpy(state).to(gpu)
    # RL
    nb_episodes = 10000
    nb_steps = 100
    gamma = 0.9
    ...
    for episode in range(nb_episodes):
        ...
        for step in range(nb_steps):
            ...
            agent_policy, agent_value, opponent_policies = agent.estimate(state.unsqueeze(0).to(gpu))
