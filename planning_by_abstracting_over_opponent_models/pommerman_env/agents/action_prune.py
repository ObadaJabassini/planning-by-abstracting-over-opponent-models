...
def _check_if_flame_will_gone(obs, prev_two_obs, flame_pos):
    ...
    if not (utility.position_is_flames(prev_two_obs[0]['board'], flame_pos) and utility.position_is_flames(
            prev_two_obs[1]['board'], flame_pos)):
        return False
    board = obs['board']
    Q = deque(maxlen=121)
    Q.append(flame_pos)
    ...
    while len(Q) > 0:
        pos = Q.popleft()
        if not (utility.position_is_flames(prev_two_obs[0]['board'], pos) and utility.position_is_flames(
                prev_two_obs[1]['board'], pos)):
            return False
        for d in dirs:
            next_pos = utility.get_next_position(pos, d)
...
def _kick_test(board, blast_st, bomb_life, my_position, direction):
    def moving_bomb_check(moving_bomb_pos, p_dir, time_elapsed):
        ...
        for i in range(10):
            ...
            if bomb_life[pos2] > 0 >= life_now >= -2 and dist < blast_st[pos2]:
                return False
